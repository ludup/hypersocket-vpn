			...
			return processDataTablesRequest(request,
					new DataTablesPageProcessor() {
						...
						public List<?> getPage(String searchPattern, int start, int length,
							...
							return websiteService.searchResources(sessionUtils.getCurrentRealm(request), searchPattern, start, length, sorting);
						}
						
						@Override
						public Long getTotalCount(String searchPattern) throws UnauthorizedException, AccessDeniedException {
							return websiteService.getResourceCount(sessionUtils.getCurrentRealm(request), searchPattern);
						}
					});
		} finally {
		...
		try {
			...
			if (resource.getId() != null) {
				newResource = websiteService.updateResource(
						...
						resource.getName(), resource.getLaunchUrl(), roles);
			} else {
				newResource = websiteService.createResource(resource.getName(),
						resource.getLaunchUrl(), roles, realm);
			}
			return new ResourceStatus<WebsiteResource>(newResource,
					I18N.getResource(sessionUtils.getLocale(request),
							WebsiteResourceServiceImpl.RESOURCE_BUNDLE, resource
